...
class Coulomb1d{
  ...
  std::vector<std::complex<double> > mel_; //elementi di matrice
...
public:
  ...
  Coulomb1d(int nspins,int Mag, double traplength=1.,double hbar=1., double mass=1.):nspins_(nspins), Mag_(Mag),
  ...
  void Init(){ //parte indep dalla configurazione
    ...
    for(int i=1;i<connectedcomponents_;i++){
      mel_[i]= KEconst_ * (1); //contributo energetico per una singola particella che si muove o uno spin che cambia stato
    }										////es C{-4,1,1,1,1}
    ...
    std::cout<<"# Coulomb potential in a trap for "<<Mag_<<" particles in one dimension with alpha = "<<alpha_<<std::endl;
  ...
  void FindConn(const std::vector<int> & stateIndex,std::vector<std::vector<int> > & flipsh,std::vector<std::complex<double> > & mel, 
    ...
    //Coulomb potential dependent on particle stateIndex
    for(int i=0;i<Mag_;i++){
      for(int j=i+1;j<Mag_;j++){
        double rab = abs(stateIndex[j]-stateIndex[i])/double(nspins_);	//traplenght=1. -> dx=1/nspins -> rab= distance*dx (distanza tra le particelle)
        mel[0] += alpha_/rab;
      }
    }
    ...
    for(int i=0; i<Mag_; i++){
      ...
      if(stateIndex[i]==0 || stateIndex[i]==nspins_-1){ //termine aggiuntivo correttivo per le particelle a inizio e fine sistema
        mel[0]+= KEconst_ * (-1);
      }
    }
    //costruisco nuovi possibili stati che hanno lo spin flip
    int index=1;
    for(int i=0; i<Mag_; i++){	  //ciclo che scorre su tutte le particelle
      for(int k=0; k<2; k++){     // 2= nn_[i].size(), ciclo che scorre sui vicini, che sono due
        if(i<Nup){
          if(nn_[i][k]>=0){ //il vicino è valido e la particella può essere spostata su quel sito
	    flipsh[index]=std::vector<int>{stateIndex[i], nn_[i][k]};
          /*std::cout<<stateIndex[i]<<"  "<<vicino<<std::endl;
          std::cout<<nn_[i][k]<<std::endl;*/
            index++;
          }
          else { //vicino non valido, quindi già occupato o fuori dai bordi
            mel[index]=0.;
  ...
  int MinFlips()const{ //mantains magnetization (se flippo due spin la magnetizzazione non cambia)
    return 2;
  }
