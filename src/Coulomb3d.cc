...
public:
  ...
  Coulomb3d(int nspins,int Mag, double traplength=1.,double hbar=1., double mass=1.):nspins_(nspins), Mag_(Mag),
	traplength_(traplength), hbar_(hbar), mass_(mass){
    connectedcomponents_ = 1+6*Mag_; //6 poichè in un reticolo 3d il numero di vicini di ogni particella è 6
    						//???
    l_= cbrt(nspins_); //cbrt è la radice cubica
  ...
  void FindConn(const std::vector<int> & stateIndex,std::vector<std::vector<int> > & flipsh,std::vector<std::complex<double> > & mel, int Nup){
    ...
    for(int i=0;i<Mag_;i++){
      for(int j=i+1;j<Mag_;j++){
        double rab = Distance(stateIndex[j],stateIndex[i])/double(l_);	//traplenght=1. -> dx=1/l_ -> rab= distance*dx
        mel[0] += double(alpha_)/rab;
      }
    }
    ...
    for(int i=0; i<Mag_; i++){
      ...
      std::vector<int> CooPart=CalculateCoo(stateIndex[i]); //CalculateCoo calcola le coordinate della particella
      for(int j=0; j<CooPart.size(); j++){			//CooPart.size()==3
        if(CooPart[j]==0 || CooPart[j]==l_-1){
          mel[0]+= KEconst_ * (-1);
        }
      }
    }
 ...
 int PbcH(int nn,int s){
  ...
  double Distance(int s1, int s2){
    std::vector<int> COOs1= CalculateCoo(s1);
    std::vector<int> COOs2= CalculateCoo(s2);
    double a = pow(COOs1[0]-COOs2[0],2);
    double b = pow(COOs1[1]-COOs2[1],2);
    double c = pow(COOs1[2]-COOs2[2],2);
